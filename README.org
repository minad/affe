...
in the background, e.g., ~find~ or ~grep~. The output produced by this process is
...
presented in the minibuffer using [[https://github.com/minad/consult][Consult]], which allows to quickly select from
the available items.
...
Affe requires the ~rg~ ("ripgrep") command line program to be available for best
performance. If ~rg~ is not available, the usual Posix ~find~ and ~grep~ utilities are
...
- [[https://github.com/minad/vertico][Vertico]] or [[https://gitlab.com/protesilaos/mct][Mct]]: Vertical completion systems
