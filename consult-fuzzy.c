...
static void wait_proc(pid_t* pid, int* fd) {
    close(*fd);
...
static void filter_end(pid_t* pid, int* fd) {
    wait_proc(pid, fd);
    chk("write failed", write(STDOUT_FILENO, "\3", 1) != 1); // ETX
}

static void filter_start(pid_t* pid, int* fd, const char* cmd, const char* filter) {
    if (*pid >= 0) {
...
int main(int argc, char* argv[]) {
    ...
    for (;;) {
        ...
        if (FD_ISSET(STDIN_FILENO, &read_fds)) {
            ...
            if (len > 1) {
                buf[len - 1] = 0;
                filter_start(&filter_pid, &filter_fd, argv[2], buf);
                data_written = 0;
            }
        }
        ...
        if (source_fd >= 0 && FD_ISSET(source_fd, &read_fds)) {
            ...
            if (len < 0)
                ...
                wait_proc(&source_pid, &source_fd);
        }
        ...
        if (filter_fd >= 0 && FD_ISSET(filter_fd, &write_fds) && data_written < data_size) {
            ...
            if (len < 0) {
                if (errno == EPIPE)
                    filter_end(&filter_pid, &filter_fd);
                else
